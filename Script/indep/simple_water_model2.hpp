/*
增加了氢键的Occuppy参数，但在使用中效果不明显
2008.10.29.
*/

# include "../../basiclib/Geometry.hpp"
using namespace std;
...
//Aromatic nitrogen
//For HSD (c1, c2, c3)顺序是：N-N-H， N-N不成键
...
//For HSP (c1, c2)顺序是：N-H，
int PositionWater6(myvector &c1, myvector &c2, vector<myvector> &ps)
{
   int k=0, n=0;
   myvector temp(3, 0), h0;
   ...
   for(k=0; k<h0.size(); k++)
   {
      if(!PointsOnLine(h0[k], c1, c2, temp))
      {
         cout<<"Error in PositionWater6\n";
         break;
      }
      ps.push_back(temp);
      n++;
...
int PlaceWater(string &res, str2vectord &coor, vector<myvector> &wats, vector<int> &param, vector<float> &Occuppy)
{
   ...
   if(res=="ASP")
   {
      ...
      param.push_back(1); param.push_back(1); 
      Occuppy.push_back(0.5); Occuppy.push_back(0.5);
      Occuppy.push_back(0.5); Occuppy.push_back(0.5);
   }
   else if(res=="ASN")
   {
   ...
   else if(res=="LYS")
   {
      ...
      Occuppy.push_back(0.33);Occuppy.push_back(0.33);Occuppy.push_back(0.33);
   }
   else if(res=="GLU")
   {
   ...
   else if(res=="HSD")
   {
      ...
      Occuppy.push_back(1.0); Occuppy.push_back(1.0);
   }
   else if(res=="HSP")
   {
      num+=PositionWater6(coor["ND1"], coor["HD1"], wats);
      num+=PositionWater6(coor["NE2"], coor["HE2"], wats);
