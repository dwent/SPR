...
int PlaceWaterOfChargedRes(string &res, str2vectord &coor, vector<myvector> &wats)
{
   if(coor.size()==0) return 0;
   int num=0;
   if(res=="ASP")
   ...
   {
      num+=PositionWater2(coor["HE2"], coor["NE2"], wats);
      num+=PositionWater2(coor["HD1"], coor["ND1"], wats);
   }
   else if(res=="SAB" || res=="SAC")
   {
      num+=PositionWater1(coor["O1A"], coor["C1"], coor["C2"], wats);
...
int PlaceWaterOfPolarRes(string &res, str2vectord &coor, vector<myvector> &wats)
{
   if(coor.size()==0) return 0;
   int num=0;
